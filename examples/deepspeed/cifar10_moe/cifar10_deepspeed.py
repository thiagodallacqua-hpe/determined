...
def main(args, info, context):
    ...
    for op in context.searcher.operations():
            ...
            for i, data in enumerate(trainloader):
                ...
                if i % args.log_interval == (
                    ...
                    if distributed.rank==0:
                        context.train.report_training_metrics(latest_batch=step, metrics={"loss": loss.item()})
            epochs_trained += 1
            if context.distributed.rank==0:
                op.report_progress(epochs_trained)

        metadata = {"latest_batch": step}
        ...
        if context.distributed.rank==0:
            with storage_manager.store_path() as (
                ...
                model_engine.save_checkpoint(
                    path, 
                    tag='cifar10', 
                    client_state={"latest_batch": step, "epochs_trained": epochs_trained}
                )
        ...
        if distributed.rank==0:
            context.train.report_validation_metrics(latest_batch=step, metrics=metrics)
            op.report_completed(accuracy)

if __name__=="__main__":
    distributed = det._core.DistributedContext.from_deepspeed()
    info = det.get_cluster_info()
    args = AttrDict(info.trial.hparams)

    with det._core.init(distributed=distributed) as context:
